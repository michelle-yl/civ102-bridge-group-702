...
# set loading for 1000 N distributed among 3 train cars.  Puts the leftmost car wheel at x = 0.
...
''' Calculate safety factors for one pass of a train '''

# Calculates minimum safety factors for each of the 8 types of FOS for a pass of the train to the end of the bridge and back
def simulation_safety_factors(loads, span, I, b=2.54):
    ...
    #   loads = [[load1, position1], [load2, position2], ...]
    # span = length of bridge (mm)
    # I: list of cross sections and their features
    #   I_list = [[I_value, geometry, (start, end), layers], ...]
    # b: width of cross-section at centroidal axis
    # Returns a dictionary of the minimum safety factors for each type of failure
    ...
    for x in range(span - loads[-1][1]):
        ...
        # cement shear stress
        # two types to consider: shear stress between glue tabs and shear stress between layers of flange
        ...
        # plate buckling
        for i in range(len(I)):
            FOS1, FOS2, FOS3, FOS4 = safety_factor_thin_plate(I[i], flex_comp, shear_stress_profile, 180)  # a must be hard-coded
        ...
        loads = update_loads(loads, "right")  # shift loads right 1 mm and loop again

    # train pass on the way back
    ...
    for x in range(span - loads[-1][1]):
        ...
        # plate buckling
        for i in range(len(I)):
            FOS1, FOS2, FOS3, FOS4 = safety_factor_thin_plate(I[i], flex_comp, shear_stress_profile, 180)
            if abs(FOS1) < min_safety_factors["case 1"]:
                min_safety_factors["case 1"] = abs(FOS1)
            if abs(FOS2) < min_safety_factors["case 2"]:
                min_safety_factors["case 2"] = abs(FOS2)
        ...
        loads = update_loads(loads, "left") # shift loads left 1 mm and loop again

    return min_safety_factors # returns a dictionary = {'flexural tension': FOS, 'flexural compression': FOS, 'shear': FOS, 'cement shear': FOS, 'case 1': FOS, 'case 2': FOS, 'case 3': FOS, 'case 4': FOS}

# Simulate minimum safety factors at every x-value for one pass of a train
def simulation_safety_factors_across_bridge(span, I, b=None):

    # Parameters:
    # span = length of bridge (mm)
    # I: list of lists of cross-sectional features (cross-sections change for different sections of the span)
    #   I = [[I1, geometry1, (start, end)], [I2, geometry2, (start, end)]...]
    # b = width of cross-section at centroidal axis (mm)
    # Returns 9 lists of x-coordinate-FOS pairs
    #   FOS_'type'_diagram = [[x1, FOS1], [x2, FOS2], ...] ---> 9 of these lists
...
if __name__ == "__main__":
    ...
    #geometry = {"A1": [(10, 0), 80, 1.27], "A2": [(10, 1.27), 1.27, 75.5], "A3": [(88.73, 1.27), 1.27, 75.5], "A4": [(0, 76.77), 10, 1.27], "A5": [(10, 76.77), 6.27, 1.27], "A6": [(83.73, 76.77), 6.27, 1.27], "A7": [(90, 76.77), 10, 1.27], "A8": [(0, 78.04), 100, 1.27], "A9": [(0, 79.31), 100, 1.27]}
    #layers = 2

    '''good final geo'''
    geometry = {"A1": [(10, 0), 80, 1.27], "A2": [(10, 1.27), 6.27, 1.27], "A3": [(83.73, 1.27), 6.27, 1.27], "A4": [(10, 2.54), 1.27, 133.65], "A5": [(88.73, 2.54), 1.27, 133.65], "A6": [(10, 136.19), 6.27, 1.27], "A7": [(83.73, 136.19), 6.27, 1.27], "A8": [(0, 137.46), 100, 1.27], "A9": [(0, 138.73), 100, 1.27]}
    ...
    # # IT2 buckling height = 75.5
    # geometry = {"A1": [(10, 0), 80, 1.27], "A2": [(10, 1.27), 1.27, 75.5], "A3": [(88.73, 1.27), 1.27, 75.5], "A4": [(10, 76.77), 6.27, 1.27], "A5": [(83.73, 76.77), 6.27, 1.27], "A6": [(0, 78.04), 100, 1.27], "A7": [(0, 79.31), 100, 1.27]}
    # layers = 2


    # #IT3 buckling height = 75.5, case 2 = 3 layers
    # geometry = {"A1": [(10, 0), 80, 1.27], "A2": [(10, 1.27), 1.27, 75.5], "A3": [(88.73, 1.27), 1.27, 75.5], "A4": [(0, 76.77), 10, 1.27], "A5": [(10, 76.77), 6.27, 1.27], "A6": [(83.73, 76.77), 6.27, 1.27], "A7": [(90, 76.77), 10, 1.27], "A8": [(0, 78.04), 100, 1.27], "A9": [(0, 79.31), 100, 1.27]}
    # layers = 2


    # # IT4
    # geometry = {"A2": [(10, 0), 1.27, 75.5], "A3": [(88.73, 0), 1.27, 75.5], "A4": [(0, 75.5), 10, 1.27], "A5": [(10, 75.5), 6.27, 1.27], "A6": [(11.27, 74.23), 77.46, 1.27], "A7": [(83.73, 75.5), 6.27, 1.27],"A8": [(90, 75.5), 10, 1.27], "A9": [(0, 76.77), 100, 1.27], "A10": [(0, 78.04), 100, 1.27]}
    # layers = 2


    #IT5
    # geometry = {"A2": [(10, 0), 1.27, 95], "A3": [(88.73, 0), 1.27, 95], "A4": [(0, 95), 10, 1.27], "A5": [(10, 95), 6.27, 1.27], "A6":[(83.73, 95), 6.27, 1.27], "A7": [(90, 95), 10, 1.27], "A8": [(0, 96.27), 100, 1.27], "A9":[(0, 97.54), 100, 1.27], "A10":[(0, 98.81), 100, 1.27]}
    # layers = 3


    # # IT7
    # geometry = {"A1": [(10, 0), 80, 1.27], "A2": [(10, 1.27),1.27, 75.5],"A3":[(88.73, 1.27),1.27,75.5],"A4":[(11.27, 66.27),77.46,1.27], "A5":[(10, 76.77), 80, 1.27], "A6": [(0, 78.04), 100, 1.27], "A7": [(0, 79.31), 100, 1.27]}
    # layers = 2


    # IT8
    # geometry = {"A2": [(10, 0), 1.27, 95], "A3": [(88.73, 0), 1.27,95], "A4": [(10, 95), 6.27,1.27], "A5": [(83.73, 95), 6.27,1.27],"A6": [(0, 96.27),100,1.27],"A7": [(0, 97.54),100,1.27],"A8":[(0, 98.81),100,1.27]}
    # layers = 3


    # # IT9
    # geometry = {"A1": [(10, 0), 80, 1.27], "A2": [(10, 1.27), 1.27, 75.5], "A3": [(88.73, 1.27), 1.27, 75.5], "A4": [(10, 76.77), 80, 1.27], "A5": [(0, 78.04), 100, 1.27], "A6": [(0, 79.31), 100, 1.27]}
    # layers = 2


    # # IT10
    # geometry = {"A1": [(0, 0),100,1.27], 'A2': [(10, 1.27),1.27,75.5],'A3':[(88.73, 1.27),1.27,75.5],'A4':[(11.27, 38.385),77.46,1.27],'A5':[(0, 76.77),100,1.27]}
    # layers = 1


    # # IT11
    # geometry = {"A1": [(10, 0), 80, 1.27], "A2": [(10, 1.27),1.27, 75.5],"A3":[(88.73, 1.27),1.27,75.5],"A4":[(11.27, 75.5),77.46,1.27], "A5":[(10, 76.77), 80, 1.27], "A6": [(0, 78.04), 100, 1.27], "A7": [(0, 79.31), 100, 1.27]}
    # layers = 2


    # # IT13
    # geometry = {'A2':[(10, 0),1.27,156.19],'A3':[(88.73, 0),1.27,156.19],'A4':[(10, 156.19),6.27,1.27],'A5':[(83.73, 156.19),6.27,1.27], 'A6': [(0, 157.46),100,1.27], 'A7':[(0, 158.73),100,1.27]}
    # layers = 2


    # # IT14 plate buckle = 134.92
    # geometry = {'A1':[(10, 0),80,1.27],'A2':[(10, 1.27),1.27,134.92],'A3':[(88.73, 1.27),1.27,134.92],'A4':[(10, 136.19),6.27,1.27],'A5':[(83.73, 136.19),6.27,1.27],'A6':[(0, 137.46),100,1.27],'A7':[(0, 138.73),100,1.27]}
    # layers = 2


    # LOAD 400
    #loads = [[400/6, 0], [400/6, 176], [400/6, 340], [400/6, 516], [400/6, 680], [400/6, 856]]
    ...
    #print(calculate_centroidal_axis(geometry))


    #I_list = [[second_moment_of_area(geometry, calculate_centroidal_axis(geometry)), geometry, (0, span), layers]]
    #min_fos = simulation_safety_factors(loads, span, I_list)
    #print(min_fos)